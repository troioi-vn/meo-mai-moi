...
ACTIVE_SLOT_FILE="$PROJECT_ROOT/.deploy-active-slot-prod"
NGINX_TEMPLATE="$PROJECT_ROOT/deploy/nginx/meo-mai-moi.com.conf.template"
NGINX_TARGET="/etc/nginx/conf.d/meo-mai-moi.com.conf"
...
read_setting() {
    ...
    read_env_value "$ROOT_ENV_FILE" "$key" "$default_value"
}

slot_backend_port() {
    case "$1" in
        a) read_setting "SLOT_A_BACKEND_HOST_PORT" "8011" ;;
        b) read_setting "SLOT_B_BACKEND_HOST_PORT" "8012" ;;
        *) return 1 ;;
    esac
}
...
slot_reverb_port() {
    case "$1" in
        a) read_setting "SLOT_A_REVERB_HOST_PORT" "8091" ;;
        b) read_setting "SLOT_B_REVERB_HOST_PORT" "8092" ;;
        *) return 1 ;;
    esac
}
