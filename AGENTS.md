...
What you see at `http://localhost:8000` is the image, not your working tree: the compiled frontend bundle **and** the PHP source are baked in at build time, and only `backend/.env`, uploads, logs, and the docs dist are bind-mounted. So redeploy before trusting anything observed there, E2E results included; a host-side `vp build` does not help. If behaviour looks impossible, check what the container is actually running: `docker compose exec backend cat /var/www/config/version.php`. This is working knowledge rather than a caveat to recite — it belongs in an answer only when the task actually turns on it.
...
- **Done means the gate passes.** `.woodpecker/test.yml` runs Pint, phpstan, deptrac, the backend suite, `i18n:ci`, `vp check`, `vp test`, and the `@smoke` Playwright tests on every PR and every push to `dev` and `main`, and GitHub requires it before a merge. A feature branch with no PR runs nothing. Run the parts your change touches locally first. The full browser suite runs after the dev deploy (`docs/e2e-ci.md`).
