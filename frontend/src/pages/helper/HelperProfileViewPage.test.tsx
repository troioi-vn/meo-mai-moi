        ...
        >
          <Routes>
            <Route
              ...
              element={
                <>
                  <HelperProfileViewPage />
                  <LocationDisplay />
                </>
              }
            />
            <Route path="/helper/:id/edit" element={<LocationDisplay />} />
          </Routes>
        </AllTheProviders>
      </MemoryRouter>
    ),
...
describe('HelperProfileViewPage - Placement Responses section', () => {
  ...
  it('shows helper breadcrumbs and routes the edit action to /helper/:id/edit', async () => {
    server.use(
      http.get('http://localhost:3000/api/helper-profiles/:id', () => {
    ...
    const { user } = renderHelperProfile()

    await waitFor(() => {
      expect(screen.getByRole('link', { name: 'My Profiles' })).toBeInTheDocument()
    })
    ...
    await waitFor(() => {
      expect(screen.getByTestId('location-display')).toHaveTextContent('/helper/1/edit')
    })
  })
})
