...
#[OA\Post(
    path: '/api/mcp/group-invitations/decline',
    summary: 'Decline a group invitation without putting its bearer token in the URL',
    ...
    responses: [
        new OA\Response(response: 204, description: 'Group invitation declined'),
        new OA\Response(response: 404, description: 'Group invitation not found'),
        new OA\Response(response: 409, description: 'Stale invitation version'),
        new OA\Response(response: 410, description: 'Group invitation is no longer valid'),
    ]
)]
class McpDeclineGroupInvitationController extends Controller
{
    use ApiResponseTrait;
    use HandlesAuthentication;
    use HandlesOfflineVersionChecks;

    public function __invoke(Request $request, ResourceInvitationService $service): JsonResponse|Response
    {
        /** @var User $user */
        ...
        if ($invitation === null || $invitation->type !== ResourceInvitationType::GROUP) {
