...
- **Email unsubscribe**: Links in notification emails use `GET /unsubscribe` (signed token) → SPA confirmation dialog → `POST /api/unsubscribe`; see [Email Unsubscribe](#email-unsubscribe) below.
...
Email notification footers include a signed unsubscribe link generated by `UnsubscribeService`. The flow is intentionally two-step so users confirm before preferences change.

### Link → SPA confirmation

1. The email link targets `GET /unsubscribe?user=…&type=…&token=…` on the app origin.
2. Laravel validates nothing at this stage; it redirects to `/settings/notifications?unsubscribe=1&…&channel=email`.
3. If the visitor is not signed in, the SPA sends them to login and preserves the redirect query string.
4. On the notifications settings page, `UnsubscribeEmailDialog` opens and asks the user to confirm unsubscribing from **all email notifications** (default `scope=all`).
5. Confirmation calls `POST /api/unsubscribe` with `user`, `type`, `token`, and optional `channel` / `scope`. The endpoint is CSRF-exempt but still requires a valid HMAC token.

`scope=type` disables only the originating notification type on the chosen channel; `scope=all` disables that channel for every notification type.

### PWA note

`/unsubscribe` is on the Workbox `navigateFallbackDenylist` so installed PWAs hit Laravel for the redirect instead of serving the cached React shell (which would 404).

### Key files

| Purpose              | Path                                                                 |
| -------------------- | -------------------------------------------------------------------- |
| Token + URL helpers  | `app/Services/UnsubscribeService.php`                                |
| Web redirect         | `routes/web.php` (`GET /unsubscribe`)                                |
| API handler          | `app/Http/Controllers/Unsubscribe/ProcessUnsubscribeController.php`  |
| SPA dialog           | `frontend/src/components/notifications/UnsubscribeEmailDialog.tsx` |
| PWA denylist         | `frontend/vite.config.ts`                                            |
