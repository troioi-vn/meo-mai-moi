...
| Habits          | Log habit activity for a day and keep it after reload                   | High     | `covered` | `frontend/e2e/habits.spec.ts`                               | Uses the day dialog on the detail page and verifies the saved state persists after page reload.                                                  |
| Habits          | Edit habit settings                                                     | High     | `covered` | `frontend/e2e/habits.spec.ts`                               | Verifies updating the habit name from the detail-page edit dialog.                                                                               |
| Habits          | Archive and restore habit                                               | Medium   | `covered` | `frontend/e2e/habits.spec.ts`                               | Exercises both archive and restore from the owner detail flow.                                                                                   |
| Habits          | Delete habit                                                            | High     | `covered` | `frontend/e2e/habits.spec.ts`                               | Confirms deletion through the destructive dialog and return to the habits list.                                                                  |
...
| Profile         | Upload avatar                                                           | Medium   | `covered` | `frontend/e2e/profile.spec.ts`                              | Includes invalid-file and size validation.                                                                                                       |
| Profile         | Replace avatar                                                          | Medium   | `covered` | `frontend/e2e/profile.spec.ts`                              |                                                                                                                                                  |
| Profile         | Remove avatar                                                           | Medium   | `covered` | `frontend/e2e/profile.spec.ts`                              |                                                                                                                                                  |
| Profile         | Change password                                                         | Medium   | `partial` | `frontend/e2e/profile.spec.ts`                              | Dialog and validation are covered; the full success path remains unit-level coverage.                                                            |
...
| Messages        | Send a message in an existing conversation                              | High     | `missing` | —                                                           | The page-level MSW test covers sending and rendering the returned message; no seeded two-user E2E journey exists yet.                            |
| Notifications   | Open notification list and mark a notification read                     | Medium   | `missing` | —                                                           | The `/notifications` page has MSW coverage for empty/list/read states; settings preferences are covered separately.                              |
...
| `frontend/e2e/profile.spec.ts`                              | Profile details, email verification, avatar management, and password-dialog validation.                      |
...
Messages and the `/notifications` page currently stop at page-level Vitest + MSW
coverage, so they intentionally have no Playwright spec in this table yet.
...
1. Decide whether verified-user email-change rejection needs its own E2E, or should remain backend/integration coverage only.
2. Revisit whether invitation decline deserves E2E coverage or can stay below the browser layer.
3. Add admin route access guard tests (non-admin blocked from admin routes).
