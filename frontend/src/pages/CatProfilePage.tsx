...
const CatProfilePage: React.FC = () => {
  ...
  React.useEffect(() => {
    async function load() {
      ...
      const accepted = cat.placement_requests.flatMap((pr) =>
  ...
  React.useEffect(() => {
    if (!cat?.placement_requests || !authUser?.id) {
          ...
          myHandover?.status === 'pending' && (
            <div className="mt-8 border rounded-lg p-4">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Please confirm the cat’s condition at meeting. Scheduled at{' '}
