...
- Effects (relationship type is determined by `placement_request.request_type`; transfer request is only a fallback):
  ...
  - For permanent placement (`request_type = permanent`): changes Pet.user_id (ownership) to the helper; updates OwnershipHistory (close previous owner record and open new one). Sets PlacementRequest status to `finalized`
  - For foster placement (`request_type = foster_free` or `foster_payed`): keeps pet ownership with the original owner, creates or ensures an active FosterAssignment (helper gets access via assignment). Sets PlacementRequest status to `active`
