...
| Medical records                            | Offline create queue via operation store; reconnect replay; pending rows in UI; update/delete/photos still require network                 | Queue update/delete metadata; files via media queue after record ID mapping |
...
- **Medical record create** (`frontend/src/hooks/useMedicalRecords.ts`) enqueues durable operations in IndexedDB when offline. Pending creates appear in the list with local placeholder IDs. Update, delete, and photo upload/delete still require network.
- **Medical record reconnect replay** (`frontend/src/offline/sync/`) replays pending medical-record creates, sends `Idempotency-Key` headers where supported, invalidates the pet medical-records query on success, and classifies errors as retryable (`pending`) or failed (validation/permanent).
...
- **Reconnect handling** (`use-sync-status.ts`) resumes pet mutations, replays pending offline operations, processes the upload queue, shows syncing/complete toasts, promotes pending photos after offline pet create, and warns on `beforeunload` when uploads or operations are still pending.
...
- **`OfflineSyncIssues`** popover lists failed and conflicted operations with retry (failed only) and discard actions. Retry re-queues the operation and triggers offline operation replay when online.
...
- **Weights:** create/update/delete offline; update conflicts surface as `conflicted` in the popover without merge UI.
- **Vaccinations:** create/update/delete offline; renew and photos still require network.
- **Medical records:** create offline; update, delete, and photos still require network.
...
1. **Shared `queue-core`** extracted from the media upload queue — **done** for media; domain operation queue **done for weights, vaccinations (create/update/delete), and medical records (create)**.
2. **Backend sync primitives** — idempotency keys **done for weight and vaccination create/update/delete, plus medical-record create**; version/`If-Match` checks and structured `409 Conflict` merge responses remain roadmap for broader conflict handling ([API Conventions](./api-conventions.md)).
3. **Local projections** — weight and vaccination lists merge pending creates/updates/deletes in their hooks; medical-record lists merge pending creates; pure projection helpers for other operations remain roadmap.
4. **Expanded domains** — weights create/update/delete **shipped**; vaccination create/update/delete **shipped**; medical-record create **shipped**; vaccination renew, medical-record update/delete/photos, and habits remain.
...
| **Operation queue**            | Durable domain write operations (weights create/update/delete today) | IndexedDB `meo-offline-operations`, not localStorage            | Yes — `clearOperations()`         |
...
- Offline vaccination renew/photo workflows, medical-record update/delete/photo workflows, or habits until those domains ship on the operation queue.
