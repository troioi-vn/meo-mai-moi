...
The repository includes three workflows under `.woodpecker/`: `test.yml` is the correctness gate for pull requests and pushes to `dev` and `main`, `deploy-dev.yml` ships `dev` alongside it, and `deploy-prod.yml` ships `main` only after the gate passes. Hostnames, SSH users, registry addresses, deployment paths, and secret values are operator-owned configuration and should be managed outside the public repository.
