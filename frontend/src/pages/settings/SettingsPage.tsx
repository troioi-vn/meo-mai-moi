...
function AccountTabContent() {
  ...
  useEffect(() => {
    ...
    emailForm.reset({
      email: hasTelegramPlaceholderEmail ? '' : (user?.email ?? ''),
    })
  ...
  const handleResendVerificationEmail = useCallback(async () => {
    ...
    try {
      ...
      toast(t('profile.emailVerificationResendSuccessTitle'), {
      ...
      toast.error(t('profile.emailVerificationResendErrorTitle'), {
  ...
  const handleSaveName = useCallback(
    async (values: NameFormValues) => {
      try {
        await updateProfile({
          data: { name: values.name, email: user?.email ?? '' },
        })
        await loadUser()
        setIsEditingName(false)
  ...
  const submitEmailChange = useCallback(
    async (nextEmail: string) => {
      ...
      try {
        await updateProfile({
          data: { name: user?.name ?? '', email: nextEmail },
        })
        ...
        if (emailChanged) {
          toast(t('profile.emailVerificationSentTitle'), {
          ...
          <div className="space-y-2">
            ...
            {showHiddenTareWeight && (
              <Button
                ...
                onClick={() => {
                  void navigate('/settings/tare-weight')
                }}
                aria-label={t('tareWeight.iconLabel')}
              >
                <Weight className="h-4 w-4" />
                <span className="sr-only">{t('tareWeight.iconLabel')}</span>
              </Button>
            )}
          </div>
...
export default function SettingsPage() {
  ...
  return (
    <PageContainer width="narrow" className="py-6 md:py-10">
