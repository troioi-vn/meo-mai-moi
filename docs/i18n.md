...
Keep product terms consistent across namespaces and locales. Canonical placement-type labels live in frontend `placement.requestTypes` and backend `messages.enums.placement_request_type`.

| Concept | EN | Notes |
| ------- | -- | ----- |
| Pet | pet | RU: питомец; UK: тваринка; VI: thú cưng |
| Helper | helper | Person offering foster/sitting/adoption help |
| Foster (Paid/Free) | Foster (Paid) / Foster (Free) | Temporary care |
| Permanent | Permanent | Long-term / adoptive placement |
| Pet Sitting | Pet Sitting | Short-term sitting |
| Group | group | Sentence case in UI copy |
| Finances | finances / finance space | Prefer over "ledger" in user-facing copy |
| Start / end a session | Sign in / Sign out | Use as two-word verbs; do not use Login, Logout, Log in, or Log out in UI copy |
| Create an account | Create account | Use for Meo Mai Moi's form and primary CTA; reserve Register and Registration for technical or admin contexts |
| Provider account creation | Sign up with Google / Sign up with Telegram | Use only for provider-specific buttons |

Style conventions for English UI copy:

- Sentence case for buttons, nav, and chrome (`Sign in`, `View all`, `Try again`)
- Keep route names, API fields, code symbols, and test helpers technical (`/login`, `/logout`, `/register`, `login()`). The glossary governs user-facing copy, not identifiers.
- Title Case only for rare proper-noun style product labels when needed
- Use `...` (three ASCII dots) for loading/progress ellipsis
- Prefer `Select` for form control prompts (`Select a date`, `Select a pet`)

When adding keys: camelCase segments (`viewAll`, `checkForm`). Snake_case is reserved for API enum parity (`foster_paid`, `medical_record`).
...
├── uk/                    # Ukrainian
└── vi/                    # Vietnamese
    ├── common.json              # Shared UI (nav, actions, errors, landing)
    ├── auth.json                # Sign in, register, password reset
    ├── pets.json                # Pet management
    ├── settings.json            # Settings pages
    ├── validation.json          # Form validation
    ├── helper.json              # Helper profiles
    ├── placement.json           # Placement requests (+ requestTypes)
    ├── habits.json              # Habits
    ├── media.json               # Media upload/crop
    ├── resourceInvitations.json # Resource invitation accept flows
    ├── groups.json              # Groups
    └── finance.json             # Shared finance spaces
```
...
├── uk/
└── vi/
    ├── messages.php              # API response messages (+ enums)
    ├── validation.php            # Validation messages
    ├── finance.php               # Finance domain messages
    ├── groups.php                # Group domain messages
    └── resource_invitations.php  # Resource invitation messages
```
