...
#[OA\Post(
    path: '/api/mcp/resource-invitations/decline',
    summary: 'Decline a pet invitation without putting its bearer token in the URL',
    ...
    responses: [
        new OA\Response(response: 204, description: 'Pet invitation declined'),
        new OA\Response(response: 404, description: 'Pet invitation not found'),
        new OA\Response(response: 409, description: 'Stale invitation version'),
        new OA\Response(response: 410, description: 'Pet invitation is no longer valid'),
    ]
)]
class McpDeclinePetInvitationController extends Controller
{
    ...
    public function __invoke(Request $request, ResourceInvitationService $service): JsonResponse|Response
    {
        ...
        if ($invitation === null || $invitation->type !== ResourceInvitationType::PET) {
