...
test.describe('Pet People', () => {
  ...
  test('allows creating and revoking an invitation link', async ({ page }) => {
    ...
    await expect(pendingSection.getByText('Editor', { exact: true })).toBeVisible({ timeout: 10000 })

    const invitationRow = pendingSection.locator('div.flex.items-center.justify-between.py-2').first()
