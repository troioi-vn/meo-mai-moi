...
describe('ThemeProvider', () => {
    ...
    render(
      <ThemeProvider defaultTheme="dark" storageKey={localStorageKey}>
        <ThemeProviderContext.Consumer>
          {({ setTheme }) => <button type="button" onClick={() => { setTheme('light'); }}>Set Light</button>}
        </ThemeProviderContext.Consumer>
      </ThemeProvider>
    );
  ...
  it('switches theme to dark', () => {
    render(
      <ThemeProvider defaultTheme="light" storageKey={localStorageKey}>
        <ThemeProviderContext.Consumer>
          {({ setTheme }) => <button type="button" onClick={() => { setTheme('dark'); }}>Set Dark</button>}
        </ThemeProviderContext.Consumer>
      </ThemeProvider>
    );
  ...
  it('switches theme to system and applies system preference', () => {
    ...
    render(
      <ThemeProvider defaultTheme="light" storageKey={localStorageKey}>
        <ThemeProviderContext.Consumer>
          {({ setTheme }) => <button type="button" onClick={() => { setTheme('system'); }}>Set System</button>}
        </ThemeProviderContext.Consumer>
      </ThemeProvider>
    );
