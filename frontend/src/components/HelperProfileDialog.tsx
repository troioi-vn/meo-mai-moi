...
export const HelperProfileDialog: React.FC<HelperProfileDialogProps> = ({
  open,
  onOpenChange,
  profile,
  transfer,
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {transfer && (
            <div className="rounded border p-3 bg-muted/50">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                <p>
                  <strong>Relationship:</strong> {transfer.requested_relationship_type ?? 'N/A'}
                </p>
                {typeof transfer.fostering_type !== 'undefined' && (
                  <p>
                    <strong>Fostering Type:</strong> {transfer.fostering_type ?? 'N/A'}
                  </p>
                )}
                {typeof transfer.price !== 'undefined' && (
                  <p>
                    <strong>Price:</strong> {transfer.price ?? 'N/A'}
                  </p>
                )}
                <p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <p>
              <strong>Has pets:</strong> {profile.has_pets ? 'Yes' : 'No'}
            </p>
            <p>
              ...
              <div className="grid grid-cols-3 gap-2">
                {profile.photos.map((ph) => {
                  const key =
                    typeof ph === 'string'
                      ? ph
                      : ph && typeof ph === 'object' && 'id' in ph
                        ? String((ph as { id: string | number }).id)
                        : ''
                  const src =
                    typeof ph === 'string'
                      ? ph
                      : ph && typeof ph === 'object' && 'path' in ph
                        ? `/storage/${(ph as { path?: string }).path ?? ''}`
                        : ''
                  return (
                    <img
                      key={key}
