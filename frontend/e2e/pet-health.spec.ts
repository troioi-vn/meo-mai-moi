...
test.describe('Pet Health', () => {
  ...
  test('allows managing the main pet health records', async ({ page }) => {
    ...
    const createMicrochipResponse = page.waitForResponse(
      (response) =>
        response.request().method() === 'POST' && /\/api\/pets\/\d+\/microchips$/.test(response.url())
    )
    ...
    await expect(updatedMicrochipItem.getByText(`Issuer: ${updatedIssuer}`, { exact: true })).toBeVisible({
      timeout: 10000,
    })
