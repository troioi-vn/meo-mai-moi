...
- **Read-Only Offline Pet Browsing**: React Query persists selected pet queries to IndexedDB for up to 24 hours, so cached pet lists/details can still open offline after a reload. Authenticated cache is cleared on logout and other auth-loss paths to avoid cross-user leakage on shared devices
