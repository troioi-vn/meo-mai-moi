...
The frontend ships separate light and dark web manifests. Theme switching between them is handled by the app-owned runtime in `frontend/src/lib/theme-runtime.ts`, which also updates `theme-color` and related DOM theme metadata.
