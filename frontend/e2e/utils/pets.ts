...
async function waitForCreatePetPageState(page: Page): Promise<'form' | 'login' | 'unknown'> {
  for (let attempt = 0; attempt < 20; attempt += 1) {
    if (await page.locator('input#name').isVisible().catch(() => false)) {
      return 'form'
    }

    if (await page.getByRole('heading', { name: /login/i }).isVisible().catch(() => false)) {
      return 'login'
    }
...
export async function createPetViaApiAndOpenProfile(
  ...
  const petId = await page.evaluate<number | null, { requestedPetName: string; requestedPetTypeName: string }>(
    async ({ requestedPetName, requestedPetTypeName }) => {
      ...
      const petTypes = Array.isArray(petTypesPayload) ? petTypesPayload : (petTypesPayload.data ?? [])
